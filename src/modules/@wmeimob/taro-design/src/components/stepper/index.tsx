...
export default class MMStepper extends PureComponent<IStepperProps, IStepperState> {
  ...
  render() {
    ...
    return <View className={classNames(styles.MMStepper, disabled ? styles.MMStepper_disabled : '')}>
      ...
      <View className={styles.MMStepper_text}>
        <Input disabled={value === min || value === max || disabled} type={(min !== undefined && min < 0) ? "text" : "number"} className={styles.MMStepper_input}
          value={value as any}
          onBlur={this.onInput} />
      </View>
