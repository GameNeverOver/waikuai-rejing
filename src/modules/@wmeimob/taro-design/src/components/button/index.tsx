 ...
 */
@autobind
export default class MMButton extends PureComponent<IButtonProps> {
  static options = {
    addGlobalClass: true
  };
  ...
  render() {
    const { loading, text, type } = this.props;
    return <View className={classnames(styles.MMButton, this.radiusClass, this.colorClass,
      ...
      <View className={styles.MMButton_content}>
        {loading && <View className={styles.MMButton_loading}>
          <MMLoading type={type === MMButtonType.Hollow ? MMLoadingType.Default : MMLoadingType.White} width={styles.loadingSize} height={styles.loadingSize} />
        </View>}
        <View>{text}</View>
        {this.props.children}
      </View>
