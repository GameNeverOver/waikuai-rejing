...
let styleNameSet = [];
function JSXToStylesSheetLoader(source, ast) {
...
function ${GET_STYLE_FUNC_NAME}(classNameExpression) { 
  ...
  `);
    ...
    try {
            ...
            ImportDeclaration(astPath) {
                ...
                if (cssIndex > -1) {
                    ...
                    if (node.importKind === 'value' && specifiers.length > 0) {
                        ...
                        node.specifiers = [
                            t.importDefaultSpecifier(t.identifier(styleIdentifierName))
                        ];
                    }
                    // 第一个引入的样式文件
                    if (cssFileCount === 0) {
                        ...
                        const webpackNode = template(`const ${styleSheetIdentifierValue} =__non_webpack_require__('${styleSheetIdentifierPath}').default`, babylon_1.default)();
        ...
        const code = better_babel_generator_1.default(ast).code;
...
exports.default = JSXToStylesSheetLoader;
